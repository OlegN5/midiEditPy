...
#path = '/Users/Oleg/Documents/12345/audio/'
#path = '/Users/Oleg/Downloads/TOP110/audio/'

(Bf2,Bfs2) = RPR_GetProjectPath("", 512)
...
for midiF in arr_txt:
    ...
    if (notes!=s4et):
        ...
        log('|| notes v midi: '+ str(notes) + ' || lyrics v midi: ' + str(lyrics) + ' || Slogov v txt: ' + str(s4et))
    ...
    if ap.info.length < midLen:
        ...
        log ('MIDI lenght more then mp3!!!')
        log ("apLen " + fName + '_0129.mp3 '+ ap.info.length)
        log ("midLen " + fName + '.mid '+ midLen)
    if overTempo:
        print('tempo in midi - OVER!!!')
        print ('changesTempo', get_tempo(mid))
        log('tempo in midi - OVER!!!')
        log ('changesTempo '+ get_tempo(mid))
    if message != '':
    ...
    if notes==s4et:
        make_sure_path_exists(path0+'render'+ os.sep)
        make_sure_path_exists(path0+'done'+ os.sep)
        mid.save(path0+'render'+ os.sep + fName + '.mid')
        ...
        if _path.exists(source_path1):
            destination_path = path0+'render'+ os.sep
        ...
        if _path.exists(source_path4):
            destination_path = path0+'done'+ os.sep
